...
// 2. 將檔案複製到指定資料夾
	...
	for (i=0; i<list.length; i++){
		var readable = fs.createReadStream(originalPath+'/'+list[i]); // 建立可讀(Readable)串流
		var writable = fs.createWriteStream(finalPath+'/'+list[i]); // 建立可寫(Writable)串流

		// 所有來自 readable 的資料，都會被寫入到 '/logs/TEST/'+arr[i]' 檔中
		readable.pipe(writable);
		console.log('file copied:'+list[i]); 
		};
